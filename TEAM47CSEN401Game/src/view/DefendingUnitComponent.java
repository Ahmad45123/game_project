		...
		choose.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(bv.getState()==0) {
					...
					if(bv.getAttackingUnit()==null) JOptionPane.showMessageDialog(null,"Please choose an attacking unit");
					else {
						try {
							bv.getBattleLog().append("\n" + bv.getAttackingUnit().getClass().getSimpleName() + " Lv. " + bv.getAttackingUnit().getLevel() + "\n" );
							bv.getBattleLog().append("Has attacked: " + z.getClass().getSimpleName() + " Lv. " + z.getLevel() + "\n" );
							bv.getBattleLog().append("Thus making it lose " + bv.getAttackingUnit().calculateAttack(z) + " Soldiers\n");
							...
							if(bv.getDefendingArmy().getUnits().size()==0) {
								int result = JOptionPane.showConfirmDialog(null, "Your army has won the battle","Manual Attack Outcome", JOptionPane.PLAIN_MESSAGE);
								Launcher.getGame().occupy(bv.getAttackingArmy(), bv.getDefendingArmy().getCurrentLocation());
								if(result==JOptionPane.OK_OPTION) Launcher.initialiseWorldMap();
							}
							bv.setAttackingUnit(null);
							bv.getBattleLog().append("\nThe Enemy's Turn: \n" );
							Unit unit1 = bv.getDefendingArmy().getUnits().get((int) (Math.random() * bv.getDefendingArmy().getUnits().size()));
							Unit unit2 = bv.getAttackingArmy().getUnits().get((int) (Math.random() * bv.getAttackingArmy().getUnits().size()));
							bv.getBattleLog().append("\n" + unit1.getClass().getSimpleName() + " Lv. " + unit1.getLevel() + "\n" );
							bv.getBattleLog().append("Has attacked: " + unit2.getClass().getSimpleName() + " Lv. " + unit2.getLevel() + "\n" );
							bv.getBattleLog().append("Thus making it lose " + unit1.calculateAttack(unit2) + " Soldiers\n");
							...
							if(bv.getAttackingArmy().getUnits().size()==0) {
								int result = JOptionPane.showConfirmDialog(null, "Your army has lost the battle","Manual Attack Outcome", JOptionPane.PLAIN_MESSAGE);
								if(result==JOptionPane.OK_OPTION) Launcher.initialiseWorldMap();
							}
